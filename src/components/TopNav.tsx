...
export default function TopNav() {
  ...
  // The game and lobby screens have their own headers; keep this nav off them.
  ...
  return (
    <div className="fixed right-0 top-0 z-30 flex items-center gap-2 p-3 text-sm">
      <Link
        href="/leaderboard"
        className="rounded-md px-3 py-1.5 font-medium text-slate-300 transition-colors hover:bg-slate-800 hover:text-white"
      >
        Leaderboard
      </Link>
      ...
      {!enabled ? (
        <span
          className="rounded-md bg-slate-800 px-3 py-1.5 text-slate-400"
        ...
        <>
          {isAdmin && (
            <Link
              href="/admin"
              className="rounded-md px-3 py-1.5 font-medium text-amber-300 transition-colors hover:bg-slate-800"
            >
              Admin
            </Link>
          )}
          ...
          >
            {user.photoURL ? (
              ...
              <span className="grid h-7 w-7 place-items-center rounded-full bg-indigo-500 text-xs font-bold text-white">
          ...
          <button
            onClick={signOutUser}
            className="rounded-md px-3 py-1.5 font-medium text-slate-400 transition-colors hover:bg-slate-800 hover:text-white"
          >
            Sign out
          </button>
        </>
